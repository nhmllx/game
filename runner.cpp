...
void *spriteThread(void *arg)
{
	...
	if (diff >= 0.043) {
		...
		g.frameno = 0;
	    if (g.frameno2 > 16)//total frame number
		g.frameno2 = 0;
	    if (g.frameno3 > 19)//total frame number
		g.frameno3 = 0;
	    timeCopy(&start, &end);
	}
...
void X11_wrapper::check_mouse(XEvent *e)
{
	...
	if (e->xbutton.button==1) {
	    ...
	    //Left button is down
	    //if (savex > g.xres*0.1 && savex < g.xres*0.95){
	    //    printf("look: %i\n", savex);
	    //   printf("looy: %i\n", savey);
	    //  exit(1);
	    // }
...
void init_opengl(void)
{
    ...
    unsigned char *data31 = new unsigned char [papple.width * papple.height * 4];
    for (int i=0; i<papple.height; i++) {
	for (int j=0; j<papple.width; j++) {
	    int offset  = i*papple.width*3 + j*3;
	    int offset2 = i*papple.width*4 + j*4;
	    data31[offset2+0] = papple.data[offset+0];
	    data31[offset2+1] = papple.data[offset+1];
	    data31[offset2+2] = papple.data[offset+2];
	    data31[offset2+3] =
		((unsigned char)papple.data[offset+0] != 255 ||
		 (unsigned char)papple.data[offset+1] != 0 ||
		 (unsigned char)papple.data[offset+2] != 0) ? 255 : 0;
	}
    }
    glGenTextures(1, &g.pappleid);
    glBindTexture(GL_TEXTURE_2D, g.pappleid);
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_NEAREST);
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, papple.width, papple.height,
	    0, GL_RGBA, GL_UNSIGNED_BYTE, data31);
    delete [] data31;
...
int lives = 6;
float slide = 0;
//float ppos[2] = [g.xres/3.2,g.yres/shake];
float ppos = g.xres/3.2;
float epos = g.xres;
...
void hats(float offsetx, float offsety)
{
    ...
    int spriteIndex = (g.frameno3 - 1) % numSprites;
...
void enemies(float offsetx, float offsety)
{
    epos = (g.xres*1.3) +slide;
    glPushMatrix();
    glTranslatef((g.xres*1.3) + slide, g.yres/offsety, 0.0f);
    glEnable(GL_ALPHA_TEST);
    glAlphaFunc(GL_GREATER, 0.0f);
    glColor4ub(255, 255, 255, 255);
    glBindTexture(GL_TEXTURE_2D, g.pappleid);
    ...
    int numSprites = 16;
    float spriteWidth = sw / numSprites;
    int spriteIndex = (g.frameno2 - 1) % numSprites;
    float tx111 = spriteIndex * spriteWidth / sw;
    float tx222 = (spriteIndex + 1) * spriteWidth / sw;
    float ty111 = 0.0f;
    float ty222 = 1.0f;
...
void render()
{
    ...
    r.left = g.xres * 0.85;
    r.center = 0;
    ggprint8b(&r, 16, 0x00ffffff, "Score : %i\n", score);
    ggprint8b(&r, 16, 0x00ffffff, "Distance : %i\n", diss);
    ...
    if (lives > 6){lives = 6;}
    if (lives < 0){lives = 0;}
    //printf("ppos %f || epos %f\n", ppos, epos);

    //--------------------------------------------enemies--------------------------------------------------------

    //if (g.keys[XK_d] == 1 && epos >= ppos)
    if (epos >= ppos)
    {
	enemies(1000, shake);
        if (g.keys[XK_d] == 1)
	...
	/*if (epos <= 250 && epos > 240){
	    shakef();
	    score++;
	    //	epos = g.xres;
	}*/
    }
	if (epos <= 270 && epos > 240){
	    shakef();
	    //	epos = g.xres;
	}
    //	epos = g.xres;
	...
	glPushMatrix();
	glColor3ub(255, 255, 255);
	glTranslatef(g.xres*0.85, g.yres/2, 0.0f);
	glEnable(GL_ALPHA_TEST);
	glAlphaFunc(GL_GREATER, 0.0f);
	glColor4ub(255,255,255,255);
	glBindTexture(GL_TEXTURE_2D, g.pappleid);
	float tx11z = 0.0f + (float)((g.frameno2-1) % 16) * ((1600.0f/16.0f)/1600.0f);
	float tx22z = tx11z + ((1600.0f/16.0f)/1600.0f);
	float ty11z = 1.0f ;
	float ty22z = ty11z + 1;
	float wwz = g.xres/16;
	float hhz = g.yres/10;
	glBegin(GL_QUADS);
	glTexCoord2f(tx11z, ty22z); glVertex2f(-wwz, -hhz);
	glTexCoord2f(tx11z, ty11z); glVertex2f(-wwz,  hhz);
	glTexCoord2f(tx22z, ty11z); glVertex2f( wwz,  hhz);
	glTexCoord2f(tx22z, ty22z); glVertex2f( wwz, -hhz);
	glEnd();
	glBindTexture(GL_TEXTURE_2D, 0);
	glDisable(GL_ALPHA_TEST);
	//
